...
# Create a slider using TKinter to change the scale of the model (Bonus for 1 extra point)
from guiV1 import SimpleGUI
gui = SimpleGUI("Assignment 3")
ScaleSlider = gui.add_slider("Scale", min_value=0.5*scale, max_value=2.0*scale, initial_value=scale)
...
while draw:
    ...
    # Update the scale of the model (Bonus for 1 extra point)
    scale = ScaleSlider.get_value()
