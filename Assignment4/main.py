...
gui = SimpleGUI("Assignment 3")

rotateZ_slider = gui.add_slider("RotateZ", min_value=-90, max_value=90, initial_value=0)
rotateY_slider = gui.add_slider("RotateY", min_value=-180, max_value=180, initial_value=0)
rotateX_slider = gui.add_slider("RotateX", min_value=-90, max_value=90, initial_value=0)
...
while draw:
    ...
    rotateZ_mat = pyrr.matrix44.create_from_z_rotation(np.deg2rad(rotateZ_slider.get_value()))
    rotateY_mat = pyrr.matrix44.create_from_y_rotation(np.deg2rad(rotateY_slider.get_value()))
    rotateX_mat = pyrr.matrix44.create_from_x_rotation(np.deg2rad(rotateX_slider.get_value()))

    model_mat = pyrr.matrix44.multiply(translation_mat, rotateZ_mat)
    model_mat = pyrr.matrix44.multiply(model_mat, rotateY_mat)
    model_mat = pyrr.matrix44.multiply(model_mat, rotateX_mat)
